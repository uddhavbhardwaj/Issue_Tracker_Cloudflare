...
// .wrangler/tmp/bundle-0Ne90U/checked-fetch.js
...
// .wrangler/tmp/bundle-0Ne90U/strip-cf-connecting-ip-header.js
...
app.post("/api/seed", async (c) => {
  const { results } = await c.env.DB.prepare(`
    ...
    RETURNING *
  `).all();
  return c.json({ message: "Seeded successfully", results });
});
app.post("/api/feedback", async (c) => {
  const { content, source } = await c.req.json();
  let aiResponse;
  try {
    const messages = [
...
Return JSON in this format: { "sentiment": "String", "urgency_score": Int, "urgency_reason": "String", "themes": ["String"] }` }
    ...
    aiResponse = await c.env.AI.run("@cf/meta/llama-3-8b-instruct", { messages });
  } catch (e) {
    return c.json({ error: "AI Analysis failed", details: e.message }, 500);
  }
  let analysis = {};
  ...
  try {
    ...
    if (jsonMatch) {
      ...
      analysis = { sentiment: "Neutral", urgency_score: 5, urgency_reason: "AI parsing failed", themes: [] };
    }
  } catch (e) {
    analysis = { sentiment: "Neutral", urgency_score: 5, urgency_reason: "AI parsing failed", themes: [] };
  }
  ...
  const { results } = await c.env.DB.prepare(`
    INSERT INTO feedback (content, source, sentiment, urgency_score, urgency_reason, themes, status)
    VALUES (?, ?, ?, ?, ?, ?, 'New')
    RETURNING *
  `).bind(content, source || "Unknown", analysis.sentiment, analysis.urgency_score, analysis.urgency_reason, themesStr).all();
...
app.get("/api/dashboard", async (c) => {
  ...
  `).all();
  const allThemes = await c.env.DB.prepare(`SELECT themes FROM feedback WHERE status != 'Archived'`).all();
  const themeCounts = {};
...
app.get("/api/inbox", async (c) => {
  const { results } = await c.env.DB.prepare(`
    SELECT * FROM feedback WHERE status != 'Archived' ORDER BY urgency_score DESC, created_at DESC
  `).all();
...
// .wrangler/tmp/bundle-0Ne90U/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-0Ne90U/middleware-loader.entry.ts
