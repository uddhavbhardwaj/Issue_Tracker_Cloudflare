...
// .wrangler/tmp/bundle-U7hf1a/checked-fetch.js
...
async function processSingleFeedback(feedbackData, env) {
...
__name(processSingleFeedback, "processSingleFeedback");
async function triggerFeedbackWorkflow(feedback, env) {
  try {
    if (env.FEEDBACK_WORKFLOW && typeof env.FEEDBACK_WORKFLOW.create === "function") {
...
__name(triggerFeedbackWorkflow, "triggerFeedbackWorkflow");
function validateFeedbackData(feedbackData) {
  ...
  if (feedbackData.content === void 0 || feedbackData.content === null) {
  ...
  if (feedbackData.file !== void 0 && feedbackData.file !== null) {
    if (!(feedbackData.file instanceof File) && !(feedbackData.file instanceof Blob)) {
      ...
      } else if (feedbackData.file.size > 5 * 1024 * 1024) {
  ...
  const allowedFields = ["content", "source", "file"];
  ...
  return {
    ...
    generalErrors,
    warnings,
    summary: hasFieldErrors || hasGeneralErrors ? `Validation failed for ${Object.keys(fieldErrors).length} field(s)` : "Valid",
...
__name(validateFeedbackData, "validateFeedbackData");

// src/batch-processor.js
function validateFeedbackItem(feedbackItem, index = 0) {
  const fieldErrors = {};
  const warnings = [];
  const generalErrors = [];
  ...
  if (feedbackItem.content === void 0 || feedbackItem.content === null) {
    fieldErrors.content = ["Field is required"];
  ...
  if (feedbackItem.metadata !== void 0) {
...
__name(validateFeedbackItem, "validateFeedbackItem");
function validateFeedbackBatch(feedbackArray) {
...
__name(validateFeedbackBatch, "validateFeedbackBatch");
function formatValidationErrors(validationResult) {
  ...
  if (validationResult.itemResults && validationResult.itemResults.length > 0) {
    ...
    response.itemsWithIssues = response.itemValidation.filter(
      (item) => !item.valid || item.warnings && item.warnings.length > 0
    );
  }
...
async function processSingleFeedback2(feedbackItem, env) {
  try {
    const feedbackData = {
      content: feedbackItem.content.trim(),
      source: feedbackItem.source.trim(),
      file: null
      // Batch processing doesn't support file uploads yet
    };
    const result = await processSingleFeedback(feedbackData, env);
    return {
      success: true,
...
__name(processSingleFeedback2, "processSingleFeedback");
async function processFeedbackBatch(feedbackArray, env) {
  ...
  for (let i = 0; i < feedbackArray.length; i++) {
    ...
    try {
      ...
      const processingResult = await processSingleFeedback2(feedbackItem, env);
      if (processingResult.success) {
        results.push({
          index: i,
          success: true,
          feedbackId: processingResult.feedbackId,
          warnings: itemValidation.warnings.length > 0 ? itemValidation.warnings : void 0
        });
        processedCount++;
      } else {
        results.push({
...
__name(processFeedbackBatch, "processFeedbackBatch");
function determineHttpStatusCode(batchResult) {
...
__name(determineHttpStatusCode, "determineHttpStatusCode");
function createErrorResponse(errorType, message, details = {}, statusCode = 500) {
  const response = {
    error: message,
    type: errorType,
    statusCode,
...
function formatBatchResponseWithStatus(batchResult) {
...
__name(formatBatchResponseWithStatus, "formatBatchResponseWithStatus");
...
app.post("/api/seed", async (c) => {
  ...
  try {
    ...
    console.log("JSON parsing failed, using hardcoded seed data:", parseError.message);
    useJsonInput = false;
  }
  if (useJsonInput && requestBody && requestBody.mockData && Array.isArray(requestBody.mockData)) {
    try {
      const mockDataArray = requestBody.mockData;
      const validation = validateFeedbackBatch(mockDataArray);
      if (!validation.success) {
      ...
      const batchResult = await processFeedbackBatch(mockDataArray, c.env);
      const response = formatBatchResponseWithStatus(batchResult);
      const statusCode = determineHttpStatusCode(batchResult);
      response.seedMessage = "Mock data seeded through production pipeline";
      return c.json(response, statusCode);
    } catch (error) {
      console.error("Mock data processing error:", error);
      const errorResponse = createErrorResponse(
        "processing_error",
        "Failed to process mock data",
    ...
    return c.json({
      message: workflowsTriggered > 0 ? "Seeded successfully (Background processing started)" : "Seeded successfully (Workflows not available in local dev)",
      results
    });
  }
});
app.post("/api/feedback", async (c) => {
  ...
  const feedbackData = { content, source, file };
  ...
  if (!validation.isValid) {
    ...
    if (validation.fieldErrors && Object.keys(validation.fieldErrors).length > 0) {
      errorResponse.fieldErrors = validation.fieldErrors;
    }
    if (validation.generalErrors && validation.generalErrors.length > 0) {
      errorResponse.generalErrors = validation.generalErrors;
    }
    if (validation.warnings && validation.warnings.length > 0) {
      errorResponse.warnings = validation.warnings;
    }
    errorResponse.details = validation.errors;
  ...
  try {
    ...
    try {
      ...
      const errorResponse = createErrorResponse(
        ...
        { message: "Request body must be valid JSON", parseError: parseError.message },
        400
      );
      return c.json(errorResponse, 400);
    }
    ...
    if (!requestBody.feedback) {
      const errorResponse = createErrorResponse(
        ...
        { message: 'Request must include a "feedback" array' },
        400
      );
      return c.json(errorResponse, 400);
    }
    const feedbackArray = requestBody.feedback;
    ...
    if (requestSize > requestSizeLimit) {
      const errorResponse = createErrorResponse(
        ...
        "Request too large",
        {
          message: `Request size (${requestSize} bytes) exceeds limit (${requestSizeLimit} bytes)`,
          requestSize,
    ...
    const batchResult = await processFeedbackBatch(feedbackArray, c.env);
    const response = formatBatchResponseWithStatus(batchResult);
    const statusCode = determineHttpStatusCode(batchResult);
    return c.json(response, statusCode);
  } catch (error) {
    ...
    const errorResponse = createErrorResponse(
      ...
      { message: "Failed to process batch request", error: error.message },
      500
    );
    return c.json(errorResponse, 500);
  }
...
app.get("/api/feedback/:id/similar", async (c) => {
  ...
  const similarIds = matches.matches.filter((m) => m.id !== id.toString()).map((m) => parseInt(m.id));
...
app.get("/api/dashboard", async (c) => {
  try {
    ...
    const topThemes = Object.entries(themeCounts).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([name, count]) => ({ name, count }));
    return c.json({
...
// .wrangler/tmp/bundle-U7hf1a/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-U7hf1a/middleware-loader.entry.ts
